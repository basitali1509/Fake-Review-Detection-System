...
const ProfileCard = ({name,expertise,rollno,image}) => {
  return (
    <div className="md:w-1/5 w-96 px-6 py-6 text-center bg-gray-800 rounded-lg lg:mt-0 xl:px-10">
      <div className="space-y-4 xl:space-y-6">
      <Typography variant='h4' className="text-white">{name}</Typography>
        <img className="mx-auto rounded-full h-36 w-36" src={image} alt="author avatar" />
        <div className="space-y-2">
          <div className="flex justify-center items-center flex-col space-y-0 text-lg font-medium leading-6">
            
            <Typography className="text-indigo-300">{expertise}</Typography>
            <Typography className="text-indigo-300">{rollno}</Typography>
